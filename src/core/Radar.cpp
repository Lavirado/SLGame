...
{ 
    ...
    // Dönüþtürülen köþe noktalarý
    CVector2D out[4];
    CVector2D in;

    // Radar noktalarýný dönüþtürme ve kontrol etme
    for (int i = 0; i < 4; i++) {
        in.x = corners[i].x;
        in.y = corners[i].y;
    ...
    // Mask vertexlerini ayarlama ve çizim
...
#ifdef FIX_BUGS
		out.x = (in.x + 0.6f) * 0.8f * SCREEN_SCALE_X(RADAR_WIDTH) + SCREEN_SCALE_X(RADAR_LEFT);
#else
